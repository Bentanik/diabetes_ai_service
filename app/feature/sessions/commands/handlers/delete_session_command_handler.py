    ...
    async def execute(self, command: DeleteSessionCommand) -> Result[None]:
        try:
            ...
            if is_session_exists == 0:
                return Result.failure(
                    message=SessionChatResult.SESSION_DELETED_FAILED.message,
                    code=SessionChatResult.SESSION_DELETED_FAILED.code,
                )

            await self.db.chat_histories.delete_many({"session_id": command.session_id})

            return Result.success(
                message=SessionChatResult.SESSION_DELETED_SUCCESS.message,
                code=SessionChatResult.SESSION_DELETED_SUCCESS.code,
            )
