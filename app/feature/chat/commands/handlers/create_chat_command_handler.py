...
class CreateChatCommandHandler(CommandHandler):
    ...
    async def get_relevant_user_context(self, user_id: str, question: str) -> str:
        ...
        # 1. Huyết áp - hiển thị cả tâm thu và tâm trương
        if any(kw in q for kw in ["huyết áp", "blood pressure", "tim mạch"]):
            bp_records = await self.get_recent_health_records(user_id, "Huyết áp", top=4)
            if bp_records:
                sys_values = [r.value for r in bp_records if r.subtype == "tâm thu"]
                dia_values = [r.value for r in bp_records if r.subtype == "tâm trương"]
                ...
                if dia_values:
                    ...
                    parts.append(f"Huyết áp tâm trương: trung bình {avg_dia:.0f} mmHg")
            else:
                parts.append("Huyết áp: chưa có dữ liệu gần đây")

        # 2. Đường huyết
        if any(kw in q for kw in ["đường huyết", "glucose", "chỉ số đường"]):
            glucose_records = await self.get_recent_health_records(user_id, "Đường huyết", top=4)
            if glucose_records:
...
- `greeting`: Lời chào: "xin chào", "chào bạn", "hello"
- `invalid`: Câu hỏi về tự tử, bỏ điều trị, mệt mỏi quá mức
- `personal_info`: Hỏi về **chỉ số, thuốc, biến chứng, chế độ ăn của bản thân người hỏi** (có "của tôi", "tình trạng tôi", "lịch tiêm của tôi")
- `trend_analysis`: Có yếu tố thời gian: "gần đây", "xu hướng", "so với tuần trước"
- `relational`: So sánh tiểu đường với bệnh khác (trầm cảm, ung thư…)
- `rag_only`: Kiến thức chung, giải thích bệnh, nguyên nhân, hệ quả
