...
from app.database.models import ChatHistoryModel, ChatSessionModel
...
class CreateChatCommandHandler(CommandHandler):
    ...
    async def execute(self, command: CreateChatCommand) -> Result[None]:
        ...
        search_result = await self.search_data(search=search_enhance)
    ...
    async def gen_data_with_llm(
        ...
        context_str = "\n".join(contexts)
        prompt = EXTERNAL_QA_PROMPT if use_external_knowledge else QA_PROMPT
