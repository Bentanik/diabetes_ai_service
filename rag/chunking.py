...
class Chunking:
    def __init__(
        ...
        if self.config.max_chunk_size > 512:
            self.logger.warning(
                f"max_chunk_size ({self.config.max_chunk_size}) > 512, setting to 400"
            )
    ...
    def _count_tokens(self, text: str) -> int:
        try:
            tokens = self.tokenizer.encode(
                text, truncation=True, max_length=512, add_special_tokens=False
            )
            return len(tokens)
    ...
    def _chunk_semantic_sync(self, block: TextBlock) -> List[Chunk]:
        ...
        if token_count <= self.config.min_chunk_size or content_type in [
            "heading",
            "code",
            "list",
        ]:
        ...
        for chunk in chunks:
            ...
            if (
                current_text
                and current_tokens + chunk_tokens <= self.config.max_chunk_size
            ):
                current_text += "\n\n" + chunk
        ...
        if current_text:
            if merged_chunks and current_tokens < self.config.min_chunk_size:
                ...
                if self._count_tokens(last_chunk_text) > self.config.max_chunk_size:
                    safe_merged = self._safe_truncate_by_chars(
                        last_chunk_text, self.config.max_chunk_size
                    )
    ...
    def _manual_split_preserve_format(self, text: str) -> List[str]:
        ...
        if len(paragraphs) > 1:
            ...
            for para in paragraphs:
                ...
                if (
                    current_chunk
                    and current_tokens + para_tokens <= self.config.max_chunk_size
                ):
                    current_chunk += "\n\n" + para
        ...
        while i < len(sentences):
            ...
            if (
                current_chunk
                and current_tokens + sentence_tokens <= self.config.max_chunk_size
            ):
            ...
            async with self.semaphore:
                ...
                try:
                    return await loop.run_in_executor(
                        self.executor, self._chunk_semantic_sync, block
                    )
                except Exception as e:
...
async def main():
    ...
    chunking_config.max_chunk_size = 512
    chunking_config.min_chunk_size = 64
    ...
    try:
        ...
        blocks: List[TextBlock] = [
            block for page_data in pages_data for block in page_data.blocks
        ]
        ...
        with open("chunks.json", "w", encoding="utf-8") as f:
            json.dump(
                [asdict(chunk) for chunk in chunks], f, ensure_ascii=False, indent=2
            )
