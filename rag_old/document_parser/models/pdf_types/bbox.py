"""
BBox Module - Bounding Box cho text regions trong PDF

Module này cung cấp BBox class để quản lý và thao tác với vùng giới hạn (bounding box) của các text regions được trích xuất từ PDF:

- Lưu trữ tọa độ vùng text (left, top, right, bottom)
- Tính toán các thuộc tính hình học (width, height, area, center)
- Kiểm tra overlap giữa các vùng text
- Tính khoảng cách và alignment giữa các text blocks
- Merge nhiều bounding boxes thành một vùng lớn hơn
- Chuyển đổi qua lại với dictionary format cho JSON serialization
"""
